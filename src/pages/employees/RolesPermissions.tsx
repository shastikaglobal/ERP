    ...
    try {
      channel = supabase
        ...
        .on('postgres_changes', { event: '*', schema: 'public', table: 'user_permissions' }, (payload: any) => {
          // If the change was made by the current user, skip the refresh to avoid race conditions!
          const grantedBy = payload.new?.granted_by || payload.old?.granted_by;
          if (grantedBy && grantedBy === session?.user?.id) {
            console.log('Realtime permission change generated by active user, skipping refresh.');
            return;
          }
          console.log('Realtime permission change detected from another user, refreshing silently...', payload);
