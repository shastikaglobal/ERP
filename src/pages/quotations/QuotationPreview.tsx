        ...
        <div className="max-w-3xl mx-auto">
          <div className="flex items-start justify-between pb-6 border-b border-border">
            <div>
              ...
              <div className="text-sm text-muted-foreground mt-1">{q.quotation_number}</div>
              <div className="text-xs text-muted-foreground mt-1">Date: {format(new Date(q.created_at), "PP")}</div>
            </div>
          ...
          <div className="grid grid-cols-2 gap-8 py-6 border-b border-border">
            ...
            <div>
              ...
              <div className="text-sm text-muted-foreground mt-1">{q.customers?.country || ""}</div>
              <div className="text-sm text-muted-foreground mt-1">Valid until: {q.valid_until ? format(new Date(q.valid_until), "PP") : "N/A"}</div>
            </div>
          </div>
          <table className="w-full mt-6 text-sm">
            ...
            <tbody>
              ...
              {q.quotation_items.length === 0 && (
                <tr>
                  <td colSpan={4} className="py-6 text-center text-muted-foreground italic">No items included in this quotation</td>
                </tr>
              )}
            </tbody>
          </table>
          <div className="mt-6 flex justify-end">
            <div className="w-64 space-y-1.5 text-sm">
              <div className="flex justify-between"><span className="text-muted-foreground">Subtotal</span><span className="tabular-nums">{q.currency} {subtotal.toLocaleString()}</span></div>
              <div className="flex justify-between"><span className="text-muted-foreground">Tax (18%)</span><span className="tabular-nums">{q.currency} {tax.toLocaleString()}</span></div>
              <div className="flex justify-between pt-2 border-t border-border font-semibold text-base"><span>Total</span><span className="tabular-nums">{q.currency} {Number(q.amount).toLocaleString()}</span></div>
            </div>
          </div>
