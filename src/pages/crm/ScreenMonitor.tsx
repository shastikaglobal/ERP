...
import { Monitor, CheckCircle, Clock, ShieldAlert, Eye, RefreshCw, User, MousePointer, Keyboard, Activity } from "lucide-react";
...
export default function ScreenMonitor() {
  ...
  const fetchInitialData = async () => {
    try {
      ...
      // 1. Fetch ALL profiles (not just BDE) — எல்லாரையும் காட்டணும்
      ...
      // 3. Map profiles → status using user_id match
      const mappedBdes: BdeStatus[] = allProfiles.map((p) => {
        // PRIMARY: match by user_id
        let userLogs = allLogs.filter((l) => l.user_id === p.id);

        // FALLBACK: match by user_name if user_id not found
        if (userLogs.length === 0) {
          userLogs = allLogs.filter(
            (l) =>
              l.user_name?.toLowerCase() === (p.full_name || "").toLowerCase()
          );
        }
        ...
        if (mostRecentLog) {
          const diffMs =
            Date.now() - new Date(mostRecentLog.created_at).getTime();
          if (mostRecentLog.event_type === "idle") {
            status = "Idle";
          } else if (diffMs < 15 * 60 * 1000) {
            // ← FIXED: 15 min window (was 5 min — too strict)
            status = "Online";
          } else {
            status = "Offline";
          }
        ...
        (payload) => {
          ...
          setBdes((prevBdes) =>
            prevBdes.map((bde) => {
              ...
              const matchByName =
                bde.name?.toLowerCase() === newLog.user_name?.toLowerCase();
              ...
              if (matchById || matchByName) {
                return {
                  ...
                  lastMouse:
                    newLog.event_type === "mouse_move"
                      ? newLog.created_at
                      : bde.lastMouse,
                  lastKeys:
                    newLog.event_type === "keypress"
                      ? newLog.created_at
                      : bde.lastKeys,
    ...
    const statusInterval = setInterval(() => {
      setBdes((prevBdes) =>
        prevBdes.map((bde) => {
          ...
          const diffMs =
            Date.now() - new Date(bde.lastActive).getTime();
          if (bde.status === "Online" && diffMs >= 15 * 60 * 1000) {
            return { ...bde, status: "Offline" };
          }
          return bde;
        })
      );
  ...
  const getIdleDuration = (lastActiveIso: string) => {
    ...
    const diffMins = Math.floor(
      (Date.now() - new Date(lastActiveIso).getTime()) / 60000
    );
    ...
    <div className="space-y-6 animate-fade-in text-foreground">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <Card className="flex items-center gap-4 p-5 bg-card/60 backdrop-blur-md border-border">
          ...
          <div>
            <div className="text-[10px] text-muted-foreground uppercase tracking-wider font-semibold">
              Total Audited Events Today
            </div>
            <div className="text-2xl font-bold font-mono mt-0.5">
              {stats.totalEventsToday} Logs
            </div>
          </div>
        </Card>
      </div>
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center p-12 space-y-3 bg-card/40 border border-border rounded-xl">
            ...
            <span className="text-xs text-muted-foreground font-medium">
              Re-syncing live terminal screens...
            </span>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {bdes.map((b) => (
              <Card
                key={b.id}
                className="p-5 flex flex-col justify-between gap-4 bg-card/60 backdrop-blur-md hover:border-primary/30 transition-all duration-300"
              >
                <div className="space-y-4">
                  <div className="flex items-start justify-between">
                    ...
                    <span
                      className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full font-semibold text-[9px] ${b.status === "Online"
                  ...
                  <div className="h-28 rounded-lg bg-neutral-900 border border-border flex flex-col items-center justify-center relative overflow-hidden">
                    ...
                    {b.status !== "Offline" ? (
                      <>
                        ...
                        <span className="text-[9px] font-mono text-muted-foreground text-center px-2 truncate max-w-[170px]">
                          {b.activeModule}
                        </span>
                      </>
                    ) : (
                  ...
                  <div className="space-y-1.5 pt-1 text-[10px] text-muted-foreground font-mono">
                    <div className="flex justify-between border-b border-border/20 pb-1">
                      <span className="flex items-center gap-1">
                        <MousePointer className="h-3 w-3" /> Mouse
                      </span>
                      <span className="text-foreground">
                        {formatTime(b.lastMouse)}
                      </span>
                    </div>
                    <div className="flex justify-between border-b border-border/20 pb-1">
                      <span className="flex items-center gap-1">
                        <Keyboard className="h-3 w-3" /> Keypress
                      </span>
                      <span className="text-foreground">
                        {formatTime(b.lastKeys)}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span className="flex items-center gap-1">
                        <Clock className="h-3 w-3" /> Idle Duration
                      </span>
                      <span className="text-foreground">
                        {b.status === "Idle"
                          ? getIdleDuration(b.lastActive)
                          : "—"}
                      </span>
                    </div>
                  </div>
                ...
                <div className="border-t border-border/20 pt-3 flex items-center justify-between text-[10px] text-muted-foreground">
                  <span className="flex items-center gap-1">
                    ...
                    {b.status !== "Offline"
                      ? `Active ${formatTime(b.lastActive)}`
                      : "Active Offline"}
                  </span>
                  {b.status !== "Offline" && (
                    <Button
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              Real-time socket transactions streamed globally from all terminals
            </p>
          </div>
          ...
          <table className="w-full text-left border-collapse text-xs">
            ...
            <tbody>
              {activityLogs.map((log) => (
                <tr
                  key={log.id}
                  className="border-b border-border/40 hover:bg-neutral-900/30 transition-colors"
                >
                  ...
                  <td className="p-3">
                    <span
                      className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full font-semibold text-[10px] ${log.event_type === "page_visit"
                          ? "bg-blue-500/10 text-blue-400 border border-blue-500/20"
                          : log.event_type === "mouse_move"
                            ? "bg-emerald-500/10 text-emerald-500 border border-emerald-500/20"
                            : log.event_type === "keypress"
              ...
              {activityLogs.length === 0 && (
                <tr>
                  ...
                  >
                    Waiting for real-time audit logs to stream...
                  </td>
                </tr>
              )}
            </tbody>
