...
async function fix() {
  ...
  console.log(`Updated ${logs?.length || 0} rows in attendance_logs.`);

  console.log("Fixing AttLogs...");
  const { data: rawLogs, error: err2 } = await supabase.from('AttLogs').select('id, LogDateTime').gte('id', 1411);
  ...
  console.log(`Updated ${rawLogs?.length || 0} rows in AttLogs.`);
}
fix();
